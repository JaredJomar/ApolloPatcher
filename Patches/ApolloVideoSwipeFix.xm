...
// "[VideoSwipeFix] " tag in each literal — pass straight through to the
// NSLog macro (header.h adds the ApolloPatcher prefix). Bare literal only:
// the NSLog macro's fmt parameter must not capture parenthesized exprs.
