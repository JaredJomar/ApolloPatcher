...
// because its scheme isn't registered in Apollo's CFBundleURLTypes (e.g.
// Dystopia's dystopia://response), or because it's an http/https URI (Reddit
// "Web app" API clients), which iOS can only route back to the app via
// Associated Domains universal links (which Apollo has no entitlement for).
//
